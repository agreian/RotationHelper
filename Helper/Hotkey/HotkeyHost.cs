...
namespace RotationHelper.Helper.Hotkey
{
    ...
    public sealed class HotkeyHost : IDisposable
    {
        ...
        {
            if (hotkey == null) throw new ArgumentNullException();
            if (hotkey.Key == 0) throw new ArgumentNullException();
            if (_hotKeys.ContainsValue(hotkey)) throw new HotkeyAlreadyRegisteredException("Hotkey already registered!", hotkey);

            var id = _idGen.Next();
            if (hotkey.Enabled) RegisterHotKey(id, hotkey);
        ...
        private void hotKey_PropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            ...
            if (kvPair.Value != null)
            {
                ...
                {
                    if (kvPair.Value.Enabled) RegisterHotKey(kvPair.Key, kvPair.Value);
                    else UnregisterHotKey(kvPair.Key);
                }
        ...
        private void RegisterHotKey(int id, Hotkey hotkey)
        {
            if ((int)_hwndSource.Handle != 0)
            {
                ...
                if (error != 0)
                {
                    ...
                    if (error == 1409) throw new HotkeyAlreadyRegisteredException(e.Message, hotkey, e);

                    throw e;
                }
            }
            else throw new InvalidOperationException("Handle is invalid");
        }
